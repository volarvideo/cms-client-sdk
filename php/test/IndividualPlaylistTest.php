...
class IndividualPlaylistTest extends PHPUnit_Framework_TestCase {
			...
			for($j = 0; $j <= 2; $j++){
				...
				if($test_case["expected"])
				{
					...
					if($test_case["empty"])
					{
						...
						$this->assertEmpty($result["playlists"], "Failure on case: ".$i.", ".$j);
					}
					else
					{
						unset($test_case["empty"]);
						$this->assertInternalType("array", $v->playlists($test_case), "Failure on case: ".$i.", ".$j);
					}
				}
				else
				{
					unset($test_case["expected"]);
					unset($test_case["empty"]);
					$this->assertFalse($v->playlists($test_case), "Failure on case: ".$i.", ".$j);
				}
			}
	}
